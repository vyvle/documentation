...
* Multi disks filesystem migration
* Multi nics
...
Some virtual machines have their file system installed across multiple disks — for example,
the root filesystem (`/`) can span `/dev/vda` and `/dev/vdb` at the same time. In that case,
a standard per-disk conversion would produce inconsistent volumes, because virt-v2v needs to
see all disks as a single logical domain to convert them correctly.
...
When this option is enabled, OS-Migrate queries the VMware API to determine the disk layout
of the source VM (bus type, device name for each disk), builds a libvirt domain XML file
that describes all disks together, and then passes it to `virt-v2v-in-place` using
`-i libvirtxml` instead of the usual `-i disk`.

==== Debugging multi-disk filesystem migrations

If the conversion fails or produces unexpected results, the following artifacts are available
on the conversion host for troubleshooting:
...
----
+
This log contains the full nbdcopy command that was used to copy each disk, as well as
the virt-v2v-in-place invocation.
...
----
+
This file describes the disk layout as OS-Migrate understood it from VMware. Check that
each disk entry has the correct device path (`source file`) and target device name (`target dev`).
If a disk is mapped to the wrong device, the converted instance may not boot.

* *nbdcopy command* — the exact `nbdcopy` invocation for each disk is printed in the log file.
You can replay it manually on the conversion host to test connectivity and data transfer
...
If the VMware vCenter or the OpenStack API endpoint uses a self-signed certificate or one issued by an authority not trusted by the conversion host, the migration will fail with an error similar to:
...
Both variables default to `false`. Set only the one that applies to your environment, or both if needed.

[WARNING]
====
Disabling certificate verification removes protection against man-in-the-middle attacks.
Only use these options in lab environments or on networks you fully control.
====
